...
1. **Nothing else is required.** No `pip install` (the file is stdlib-only), no `.env`, no API key,
   ...
   IP, and **29.4 s is the clean-path time.** If the tier is throttling when you run, the
...
| | |
|---|---|
| Wall clock | **29.4 s** — clean path, no backoff fired |
...
| Tests | **110** (105 offline, 5 live), 15 named after the defect they pin |
...
make test                                        # 105 offline tests, 100% of scripts/
...
| **API feedback for CMC** | [FEEDBACK.md](FEEDBACK.md) — seven dated, evidenced findings |
...
| **Demo video** | [youtu.be/I1yOhXSutAk](https://youtu.be/I1yOhXSutAk) — 2:45, captioned. The run at 0:39 is real time, not sped up |
